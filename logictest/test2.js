...
function counter(arr) {
  ...
  for (let i = 0; i < arr.length; i++) {
    //cek apakah array sama dengan array sebelumnya
    if (arr[i] === arr[i + 1]) {
      //jika sama, maka counter bertambah
      count++;
    } else {
      //jika tidak sama, maka counter bertambah 1
      count++;
      //push ke array item
      item.push(arr[i]);
      //push ke array count
  ...
  //loop untuk memasukkan nilai ke object result
  for (let j = 0; j < item.length; j += 2) {
    result[item[j]] = item[j + 1];
  }
